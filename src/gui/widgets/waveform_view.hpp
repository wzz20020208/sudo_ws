...
// 通用实时波形控件：三路滚动曲线（电压 / 角度 / 电流），分三个子图各用各的 Y 轴，
...
//  - 三路量纲差异大，分三个子图；电压固定量程，角度/电流自动缩放（实现见 .cpp）。
...
public:
    ...
    void append_sample(double voltage_v, double angle_deg, double iq_a);  // 仅主线程调用
...
private:
    enum class WaveChannel { Voltage, Angle, Current };
